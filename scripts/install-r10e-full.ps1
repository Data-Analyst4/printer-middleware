...
function Invoke-Cf {
    param([string]$Exe, [string[]]$CfArgs)
    $prev = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    $lines = & $Exe @CfArgs 2>&1 | ForEach-Object { "$_" }
    $code = $LASTEXITCODE
...
function Get-TunnelId {
    ...
    $result = Invoke-Cf -Exe $Exe -CfArgs @("tunnel", "list")
...
function Ensure-Venv {
    ...
    & $venvPython -m pip install -r $req *>$null
...
& $Nssm set $ServiceName AppParameters "main.py --host $HostAddr --port $Port" 2>&1 | Out-Null
& $Nssm set $ServiceName AppDirectory $TargetDir 2>&1 | Out-Null
& $Nssm set $ServiceName DisplayName "Printer Middleware R10E" 2>&1 | Out-Null
& $Nssm set $ServiceName Description "R10E printer middleware (P1) on port $Port -> $Hostname" 2>&1 | Out-Null
& $Nssm set $ServiceName Start SERVICE_AUTO_START 2>&1 | Out-Null
& $Nssm set $ServiceName AppThrottle 1500 2>&1 | Out-Null
& $Nssm set $ServiceName AppExit Default Restart 2>&1 | Out-Null
& $Nssm set $ServiceName AppRestartDelay 5000 2>&1 | Out-Null
& $Nssm set $ServiceName AppStdout (Join-Path $LogsDir "service-output.log") 2>&1 | Out-Null
& $Nssm set $ServiceName AppStderr (Join-Path $LogsDir "service-error.log") 2>&1 | Out-Null
...
& $Nssm set $ServiceName AppNoConsole 1 2>&1 | Out-Null
...
if (-not (Test-Path $certPath)) {
    ...
    $login = Invoke-Cf -Exe $cf -CfArgs @("tunnel", "login")
    Log $login.Output
    if (-not (Test-Path $certPath)) {
...
if (-not $tunnelId) {
    ...
    $create = Invoke-Cf -Exe $cf -CfArgs @("tunnel", "create", $TunnelName)
    Log $create.Output
    if ($create.ExitCode -ne 0 -and $create.Output -notmatch $TunnelName) {
...
Log "Routing DNS $Hostname -> tunnel $tunnelId (UUID to avoid name clash with r10-printer)"
$dns = Invoke-Cf -Exe $cf -CfArgs @("tunnel", "route", "dns", "--overwrite-dns", $tunnelId, $Hostname)
Log $dns.Output
if ($dns.ExitCode -ne 0 -and $dns.Output -notmatch "already exists|Record already exists|CNAME|Added CNAME") {
...
& $Nssm install $CfServiceName $cf 2>&1 | Out-Null
if ($LASTEXITCODE -ne 0) { throw "nssm install $CfServiceName failed" }
& $Nssm set $CfServiceName AppParameters $cfArgs 2>&1 | Out-Null
& $Nssm set $CfServiceName AppDirectory $TargetDir 2>&1 | Out-Null
& $Nssm set $CfServiceName DisplayName "Cloudflared Tunnel R10E" 2>&1 | Out-Null
& $Nssm set $CfServiceName Description "Cloudflare tunnel for $Hostname" 2>&1 | Out-Null
& $Nssm set $CfServiceName Start SERVICE_AUTO_START 2>&1 | Out-Null
& $Nssm set $CfServiceName AppExit Default Restart 2>&1 | Out-Null
& $Nssm set $CfServiceName AppRestartDelay 5000 2>&1 | Out-Null
& $Nssm set $CfServiceName AppStdout (Join-Path $LogsDir "cloudflared-output.log") 2>&1 | Out-Null
& $Nssm set $CfServiceName AppStderr (Join-Path $LogsDir "cloudflared-error.log") 2>&1 | Out-Null
& $Nssm set $CfServiceName AppRotateFiles 1 2>&1 | Out-Null
& $Nssm set $CfServiceName AppNoConsole 1 2>&1 | Out-Null
...
& $Nssm start $CfServiceName 2>&1 | Out-Null
