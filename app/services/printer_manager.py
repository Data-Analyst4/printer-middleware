...
def handle_print_request(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # v1.3.0 default: camera POST in background so a dead camera cannot delay TCP send.
    # Set CAMERA_IMPORT_ASYNC=false to restore v1.2.0 blocking POST-before-print.
    ...
    if cmd_name == "DATA" and camera_flow == "immediate":
        ...
        if cam_url is not None:
            camera_fn = (
                start_camera_import_immediate
                if CAMERA_IMPORT_ASYNC
                else forward_camera_import_immediate
            )
            camera_import_meta = camera_fn(
                job_id,
                data,
                command.get("data"),
                printer_id=printer_id,
